...
- **Config:** repo root `clankgster.config.ts`
- **Sync** materializes agent-specific trees, manifests, and marketplace entries from those inputs

Do **not** manually link or copy content into agent-native dirs to “fix” discovery — extend or fix **`@clankgster/sync`** instead.

## After any create / edit / delete under `.clank/` or `clankgster.config.ts`

1. Open **`packages/clankgster-sync/package.json`** and read the **`clankgster-sync:run`** (and `:clear` if needed) script — **do not** assume line numbers or script names; they can change
