# Writing rules, commands, and skills (clankgster-sync)
...
- DRY: ❌ rule file pastes full "How to run sync" from README → ✅ rule says "Run sync: see [references](./clankgster-sync-sync-behavior-and-config.md)" or [plugin README](../README.md)
...
Unique names across plugins: prefix `rules/`, `commands/`, `skills/`, `agents/` files with the plugin name. Full rule: [clankgster-sync-file-naming.md](../rules/clankgster-sync-file-naming.md).

Base new plugins on the clankgster-sync driver layout. **Never hand-mirror into agent dirs** — [clankgster-sync-trust-sync-and-sources.md](./clankgster-sync-trust-sync-and-sources.md).
