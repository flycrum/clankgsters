---
name: clankgsters-sync-add-new-skill
description: >-
...
Skills follow the [Agent Skills open standard](https://agentskills.io/specification): one directory per skill with required `SKILL.md` and optional `scripts/`, `references/`, `assets/`. Product-specific paths and frontmatter: [repo docs index](../../references/clankgsters-sync-repo-docs-index.md) → `docs/CURSOR-CLAUDE-CODEX-SKILLS.md`.

## Steps

1. Choose flat or plugin scope; prefix folder name with plugin scope when it avoids collisions per [clankgsters-sync-file-naming](../../rules/clankgsters-sync-file-naming.md)
...
4. Keep the skill focused: one concern, clear when-to-use; link to rules or [references](../../references/README.md) for detail per [writing conventions](../../references/clankgsters-sync-writing-conventions.md)
5. **Never** hand-link into `.cursor/skills` or `.claude/skills` — `@clankgsters/sync` mirrors from the sources above
6. After changes: [trust sync](../../references/clankgsters-sync-trust-sync-and-sources.md)
