#!/usr/bin/env node
/**
 * Published CLI entry for `@clankgsters/sync`: runs the sync-all entry with the **caller’s cwd** as repo root (via `CLANKGSTERS_REPO_ROOT`).
 *
 ...
 * - Sets `CLANKGSTERS_REPO_ROOT` to `process.cwd()` so `path-helpers` treats the invocation directory
...
const result = spawnSync(
  ...
  ['--import', pathToFileURL(tsxLoader).href, scriptPath],
  {
    stdio: 'inherit',
    cwd: process.cwd(),
    env: { ...process.env, CLANKGSTERS_REPO_ROOT: process.cwd() },
  }
);
