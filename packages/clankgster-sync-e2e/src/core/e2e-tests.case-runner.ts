 ...
 * `CLANKGSTER_REPO_ROOT` set to the case sandbox. Compares `expectedManifestPath` and `expectedFileStructurePath` to the
...
export async function runOneE2eTestsCase(
  ...
  const commandEnv = {
    ...process.env,
    CLANKGSTER_REPO_ROOT: sandboxRoot,
  };
  ...
  const expected = clankgsterIdentity.resolveFixtureStrings(
  ...
  const expectedFileStructure = clankgsterIdentity.resolveFixtureStrings(
