---
name: clankgster-e2e-debug-case
...
1. Re-run the case so the harness refreshes **`sandboxes/.e2e-tests.run-results/case-*-<name>/`** (from `packages/clankgster-sync-e2e`):
...
2. **Expected:** `src/test-cases/<name>/case-sync-manifest.json`  
   **Actual:** `sandboxes/.e2e-tests.run-results/case-*-<name>/.clankgster-cache/sync-manifest.json` (unless `syncManifestPath` overrides)
...
5. **Logging:** set `loggingEnabled: true` in the case config, or `CLANKGSTER_LOGGING_ENABLED=true`, then inspect **`.clank/logs/clankgster-sync.log`** under the sandbox root if sync behavior is wrong.
