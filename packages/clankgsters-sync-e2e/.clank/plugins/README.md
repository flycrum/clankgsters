...
| **clankgsters-e2e-create-case** | Add a new case: follow `src/test-cases/`, add config + fixtures, then run run-one.                                     |
...
This plugin lives under `packages/clankgsters-sync-e2e/.clank/plugins/`. Test cases: `src/test-cases/`. Case outputs are written to `sandboxes/.e2e-tests.run-results/case-{n}-{name}/` and retained for all pass/fail runs until the next harness run.
