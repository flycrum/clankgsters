...
| **Rules**         | `rules/` (.mdc, .md)                                  | _(no plugin rules)_                      | Cursor-only in plugins                            |
...
| **LSP**           | _(not in Cursor plugin docs)_                         | `.lsp.json` / `lspServers`               | Claude-only in plugins                            |
| **Settings**      | _(not in Cursor plugin docs)_                         | `settings.json` (e.g. default agent)     | Claude-only in plugins                            |
| **Output styles** | _(none)_                                              | `outputStyles` in manifest               | Claude-only                                       |
...
_This document was fact-checked against the linked official documentation. Claims in sections 1–14 are attributed to those sources; anything not stated there is called out in [Not in official documentation (but known to be true)](#not-in-official-documentation-but-known-to-be-true)._
