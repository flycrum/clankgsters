...
- [ ] Tests pass (or changes are intentional). Typical **`vp`** flows:
  - Full monorepo: `vp run @clankgster/sync#build && vp run -r test`
  - `@clankgster/sync` only: `vp run --filter @clankgster/sync test` (`vp test` in that package)
  - E2E package: `pnpm e2e-tests:run` (harness) or `vp run --filter @clankgster/sync-e2e test` (Vitest in that package)
